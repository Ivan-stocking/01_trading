...
本 skill 是 [01_trading](file:///Users/ivan/Documents/trae_work/01_trading) 项目的调用指引。该项目在每日 10:00 手动触发，筛选具备 3-5 日连续上涨潜力的 A 股个股，输出前 5 只个股（行业板块单维度，仅沪深主板）。
...
每日 10:00 手动触发，扫描当日开盘后前 30 分钟符合条件的个股（仅沪深主板 60/00 开头，行业板块取申万二级涨幅前 25）。
...
**输出**：
- 控制台表格（行业板块单维度，前 5 只个股）
- 顶部列出前 25 个申万二级行业及其涨幅
- 字段：排名 | 股票名称 | 代码 | 当前涨跌幅 | 所属板块 | 板块涨跌幅 | 乖离率 | 最近涨停日 | 仓位建议
- 日志写入 `stock_filter.log`
- 全量分析记录（含失败原因）追加写入 `stock_analysis_records.csv`
...
**输出**：行业板块维度通过数 + 个股详情。日志写入 `backtest.log`。
...
### 分析记录文件（stock_analysis_records.csv）

每次运行 `main.py` 时，所有分析过的股票（含失败股票）会追加写入 `stock_analysis_records.csv`，共 12 列：
...
| 代码 / 名称 / 所属板块 | 个股标识 |
| 当前涨跌幅(%) / 流通市值(亿) | 基础行情 |
| 失败原因 | 筛选阶段第一失败原因（前置展示），通过则显示"通过" |
| 最近涨停日 | 近 20 日内最近一次涨停日（仅涨停基因检查通过的股票有值） |
| 综合评分 | ranking_score |
| filter_stock是否通过 | 是/否 |
| 分钟条件是否通过 | 是/否（仅 filter_stock 通过的股票才会执行分钟检查） |
| 分钟失败原因 | 分钟条件失败的具体原因 |

**字段为空的说明**：`filter_stock` 采用短路式设计，每个条件失败立即 return，后续指标不计算。因此失败股票的"最近涨停日""综合评分"等字段为空属正常现象，不代表数据缺失。
...
| 参数 | 含义 | 默认值 |
|------|------|--------|
| `MIN_CIRCULATION_MKT_CAP` | 流通市值下限（亿） | 80 |
| `MAX_PLATE_RANK` | 行业板块排名上限（申万二级，取前N） | 25 |
| `TOP_STOCK_MAX` | 最终输出个股数量上限 | 5 |
...
| `STOCK_PREFIX_MAIN` | 允许的股票代码前缀（仅沪深主板） | `['60', '00']` |
| `STOCK_FILTER_MAX_WORKERS` | 个股筛选并发数 | 8 |
| `PLATE_CONS_MAX_WORKERS` | 板块成分股并发获取数 | 10 |
...
| 数据源 | 用途 | 节流间隔 |
|--------|------|---------|
| 申万指数（akshare `index_realtime_sw` / `index_component_sw`） | 申万二级行业涨幅排名、成分股映射 | 1.0s（default） |
| 新浪（akshare） | 实时行情、分时数据（降级）、日线数据（降级） | 1.0s |
| 东财（akshare） | 实时行情（含市值）、日线、分时（主接口）、全A指数 | 2.0s |
| 同花顺（akshare） | 概念板块（代码保留，已不调用） | 1.0s |

**降级机制**：
- 东财 push2 API 受 IP 级风控，启动时检测一次，不可用则全局禁用东财接口
- 板块数据/成分股使用申万接口，不依赖东财，稳定性高
- 实盘/日线/分时等接口在东财不可用时自动降级到新浪
- 板块成分股映射为空时进入降级模式（遍历涨幅前 100 只主板股票）
...
01_trading/
├── config.py                  # 配置中心（所有阈值）
├── main.py                    # 主程序入口（实盘筛选 + CSV 记录）
├── test_backtest.py           # 回测 + 诊断脚本（--debug-top）
├── plate_analyzer.py          # 板块分析（申万二级行业）
├── stock_filter.py            # 股票筛选（趋势、基因、强度等）
├── minute_data_processor.py   # 分时数据处理（东财主接口）
├── data_source.py             # 数据源管理（节流、东财可用性检测）
